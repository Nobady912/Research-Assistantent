...
#For whoever read this after 2024/05/18 please check the the defalut download OECD package
...
library(OECD)
#lode the function
source("/Users/tie/Documents/GitHub/RA-data-cleaning-work/The data process code/function/clean_oecd.R")
################################################################################
#API
################################################################################
...
observations_count <- wide_data %>%
  filter(!is.na(V) & !is.na(LR)) %>%
  group_by(REF_AREA) %>%
  summarize(num_observations = n(), .groups = 'drop')

# Filter out countries with less than 25 observations
countries_to_delete <- observations_count %>%
  filter(num_observations < 25) %>%
  pull(REF_AREA)

# Print the list of countries to be deleted
