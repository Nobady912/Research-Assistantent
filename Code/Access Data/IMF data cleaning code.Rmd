...
#so, the idea here is 
  #access the data for 10 countries each
  #within each group, random shorter sleep
  #if wrong, try three times
  #a big sleep 
  #keep going 

# set staff down for access the data base
databaseID <- "IFS"
startdate <- "1950-01-01"
enddate <- "2023-12-31"
checkquery <- FALSE

# list the things we need
indicators <- c(
...
# 对应的国家名称列表
# the countries list with the full name
country_names <- c(
...
# start with a empty matrix to store the data
results <- data.frame(
...
# in case the download fails, we will retry a few times
# yes, it will block you if you access too frequence.
fetch_data <- function(country_code, country_name, indicator) {
  queryfilter <- list(
    CL_FREA = "A",  # 年度数据 annual data 
    CL_AREA_IFS = country_code,  # 当前国家代码 current code 
    CL_INDICATOR_IFS = indicator  # 当前指标 current indicator 
  )
...
#rename the code so I can use the old code again.
  #(no, I cannot. I have to rewrite the code)
  #(each government organization have their own way to name countries)
  #yes, its hurt me so badly
